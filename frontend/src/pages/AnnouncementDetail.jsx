...
export default function AnnouncementDetail() {
  ...
  async function loadAnnouncement() {
    try {
      ...
      setError(error?.response?.data?.error || 'Failed to load announcement');
    } finally {
      setLoading(false);
    }
  }
  ...
  const handleAddComment = async () => {
    ...
    try {
      await client.post(`/announcements/${id}/comments`, { content: commentContent.trim() });
      setCommentContent('');
  ...
  if (loading) {
    return (
      <div className="container" style={{ padding: '2rem 1rem' }}>
        <div style={{ textAlign: 'center', color: 'var(--muted)' }}>Loading announcement...</div>
      </div>
    );
  }
        ...
        <div style={{ padding: '2rem' }}>
          ...
          <div style={{ 
            ...
            {announcement.createdBy && (
              <div>By: {announcement.createdBy?.displayName || announcement.createdBy?.username || 'Unknown'}</div>
            )}
          </div>
          <div style={{ 
            color: 'var(--text)', 
