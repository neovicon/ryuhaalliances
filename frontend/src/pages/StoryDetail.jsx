...
export default function StoryDetail() {
  ...
  async function loadStory() {
    try { setLoading(true); setError(null); const { data } = await client.get(`/stories/${id}`); setStory(data.story); } catch (err) { console.error('Error loading story:', err); setError(err?.response?.data?.error || 'Failed to load story'); } finally { setLoading(false); }
  }

  const handleAddComment = async () => {
    if (!commentContent.trim() || !user) return;
    setSubmittingComment(true);
    try { await client.post(`/stories/${id}/comments`, { content: commentContent.trim() }); setCommentContent(''); await loadStory(); } catch (err) { console.error('Error adding comment:', err); alert(err?.response?.data?.error || 'Failed to add comment'); } finally { setSubmittingComment(false); }
  };
  ...
  return (
    <div className="container" style={{ padding: '2rem 1rem' }}>
      ...
      <div className="card" style={{ marginBottom: '1.5rem', border: '1px solid rgba(148,163,184,0.2)', padding: 0, overflow: 'hidden' }}>
        {story.imageUrl && (
          <div style={{ width: '100%', aspectRatio: '16 / 10', background: `url(${story.imageUrl}) center/cover no-repeat`, backgroundSize: 'cover', borderBottom: '1px solid rgba(148,163,184,0.12)' }} />
        )}
        <div style={{ padding: '2rem' }}>
          <h1 className="hdr" style={{ fontSize: '2rem', marginBottom: '0.5rem' }}>{story.title}</h1>
          <div style={{ color: 'var(--muted)', fontSize: '0.9rem', marginBottom: '1.5rem', display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
            {story.createdAt && (<div>Created: {new Date(story.createdAt).toLocaleDateString()}</div>)}
            {story.createdBy && (<div>By: {story.createdBy?.displayName || story.createdBy?.username || 'Unknown'}</div>)}
          </div>
          <div style={{ color: 'var(--text)', fontSize: '1rem', whiteSpace: 'pre-wrap', lineHeight: '1.8' }}>{story.content}</div>
        </div>
      </div>

      <div className="card" style={{ border: '1px solid rgba(148,163,184,0.2)' }}>
        <h3 className="hdr" style={{ marginBottom: '1rem', fontSize: '1.3rem' }}>Comments ({story.comments?.length || 0})</h3>
