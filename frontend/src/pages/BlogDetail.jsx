...
export default function BlogDetail() {
  ...
  const handleAddComment = async () => {
    ...
    try {
      await client.post(`/blogs/${id}/comments`, { content: commentContent.trim() });
      setCommentContent('');
      await loadBlog();
    } catch (error) {
  ...
  if (loading) {
    return (
      <div className="container" style={{ padding: '2rem 1rem' }}>
        <div style={{ textAlign: 'center', color: 'var(--muted)' }}>Loading blog...</div>
      </div>
    );
  }
      ...
      <div className="card" style={{ marginBottom: '1.5rem', border: '1px solid rgba(148,163,184,0.2)', padding: 0, overflow: 'hidden' }}>
        {blog.imageUrl && (
          <div style={{
            width: '100%',
            aspectRatio: '16 / 10',
            background: `url(${blog.imageUrl}) center/cover no-repeat`,
        ...
        <div style={{ padding: '2rem' }}>
          ...
          <div style={{ 
            ...
            {blog.createdBy && (
              <div>By: {blog.createdBy?.displayName || blog.createdBy?.username || 'Unknown'}</div>
            )}
          </div>
          <div style={{ 
            color: 'var(--text)', 
