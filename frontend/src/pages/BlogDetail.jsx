  ...
  return (
    <div className="container" style={{ padding: '2rem 1rem', maxWidth: '900px', margin: '0 auto' }}>
      ...
      <div className="card" style={{ marginBottom: '1.5rem', border: '1px solid rgba(148,163,184,0.2)', padding: 0, overflow: 'hidden' }}>
        ...
        <div style={{ padding: '2.5rem' }}>
          ...
          <div style={{ color: 'var(--muted)', fontSize: '0.95rem', marginBottom: '2rem', display: 'flex', gap: '1.5rem', flexWrap: 'wrap', paddingBottom: '1.5rem', borderBottom: '1px solid rgba(148,163,184,0.1)' }}>
            {blog.createdAt && <div>📅 {new Date(blog.createdAt).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</div>}
            {blog.createdBy && <div>✍️ {blog.createdBy?.displayName || blog.createdBy?.username || 'Unknown'}</div>}
          </div>
          ...
          <div style={{ borderTop: '1px solid rgba(148,163,184,0.1)', paddingTop: '1.5rem', display: 'flex', flexWrap: 'wrap', gap: '1rem', alignItems: 'center', justifyContent: 'space-between' }}>
            <Reactions
              contentType="blogs"
              contentId={blog.id || blog._id}
              reactions={blog.reactions || []}
              user={user}
              onReactionUpdate={loadBlog}
            />
            <ShareButton
      ...
      <div className="card" style={{ border: '1px solid rgba(148,163,184,0.2)' }}>
        <h3 className="hdr" style={{ marginBottom: '1.5rem', fontSize: '1.5rem' }}>Comments ({blog.comments?.length || 0})</h3>
        ...
        {blog.comments && blog.comments.length > 0 ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            {blog.comments.map((comment, index) => (
              <div key={comment._id || index} style={{ border: '1px solid rgba(148,163,184,0.2)', borderRadius: '12px', padding: '1.25rem', background: 'rgba(15,23,42,0.3)', transition: 'background 0.2s' }} onMouseEnter={(e) => e.currentTarget.style.background = 'rgba(15,23,42,0.4)'} onMouseLeave={(e) => e.currentTarget.style.background = 'rgba(15,23,42,0.3)'}>
