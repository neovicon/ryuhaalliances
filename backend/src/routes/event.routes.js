...
import { requireAuth, requireAesther } from '../middleware/auth.js';
...
router.post('/', requireAuth, requireAesther, uploadImage.single('image'), uploadToStorage, async (req, res, next) => {
...
router.put('/:id', requireAuth, requireAesther, uploadImage.single('image'), uploadToStorage, async (req, res, next) => {
  // Add imageUrl to req.body if file was uploaded (store only filename/path, not full URL)
  if (req.file) {
    // Use storagePath from Supabase (filename only), otherwise fall back to local path
    req.body.imageUrl = req.file.storagePath || `/uploads/${req.file.filename}`;
