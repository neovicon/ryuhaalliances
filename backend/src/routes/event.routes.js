...
import { requireAuth, requireAestherOrEmissary, optionalAuth } from '../middleware/auth.js';
...
router.post('/', requireAuth, requireAestherOrEmissary, uploadImage.single('image'), handleMulterError, uploadToStorage, async (req, res, next) => {
...
router.put('/:id', requireAuth, requireAestherOrEmissary, uploadImage.single('image'), handleMulterError, uploadToStorage, async (req, res, next) => {
  // Add imageUrl to req.body if file was uploaded (store only filename/path, not full URL)
  if (req.file) {
    // Use storagePath from Supabase (filename only), otherwise fall back to local path
    req.body.imageUrl = req.file.storagePath || `/uploads/${req.file.filename}`;
...
router.delete('/:id', requireAuth, requireAestherOrEmissary, validateDeleteEvent, deleteEvent);
